...
describe("Token", async function () {
  ...
  async function initToken() {
    const ProjectNFT1155 = await ethers.getContractFactory(
      "ProjectNFT1155"
    );
    const token = await ProjectNFT1155.deploy();
  ...
  async function initAuction() {
    ...
    const ProjectAuction = await ethers.getContractFactory(
      "ProjectAuction"
    );

    const receiverOfFees = signers.slice(-1)[0]

    return { auction: (await ProjectAuction.deploy(receiverOfFees.address, 4)), receiverOfFees };
  }
