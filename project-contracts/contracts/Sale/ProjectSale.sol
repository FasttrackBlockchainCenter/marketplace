...
interface IProjectNFT1155 {
    function creators(uint256) external returns (address);
}

contract ProjectMarketplace is Ownable, ReentrancyGuard {
    ...
    /// @notice Project721 contract
    address public Project721;
    ...
    function buyItem(
        ...
        require(feeTransferSuccess, "ProjectMarketplace: Fee transfer failed");

        // Send royalty to creator(minter)
        if (
            IProjectNFT1155(_nftAddress).creators(_tokenId) != address(0) &&
            royalties[_tokenId] != uint8(0)
        ) {
            uint256 royaltyFee = msg.value.sub(feeAmount).mul(royalties[_tokenId]).div(100);
            (bool royaltyTransferSuccess, ) = payable(IProjectNFT1155(_nftAddress).creators(_tokenId))
        ...
        require(ownerTransferSuccess, "ProjectMarketplace: Owner transfer failed");
    ...
    function registerRoyalty(
        ...
        if (IERC165(_nftAddress).supportsInterface(INTERFACE_ID_ERC1155)) {
            require(
                IProjectNFT1155(_nftAddress).creators(_tokenId) == _msgSender(),
